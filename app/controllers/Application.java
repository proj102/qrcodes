...
public class Application extends Controller {
	...
	public static Result index() {
		return ok(index.render(Login.getCustSession(), Login.loginForm, InfoDisplay.NONE, null));
	}
	...
	public static Result redirection(String id) {
		...
		try {
			...
			return badRequest(index.render(Login.getCustSession(), Login.loginForm, InfoDisplay.ERROR, "Redirection failed. Reason : " + e));
		}
	}
	...
	public static Result myQrTable() {
		...
		try {
			...
			return badRequest(index.render(Login.getCustSession(), Login.loginForm, InfoDisplay.ERROR, "Impossible to get the Qrcodes. " + e));
		}
	}
	
	public static Result overview() {
		return ok(overview.render(Login.getCustSession(), Login.loginForm));
	}
	
	public static Result createQr() {
		return ok(createQr.render(Login.getCustSession(), Login.loginForm, urlForm, InfoDisplay.NONE, null));
	}
	...
	public static Result getUrl() {
		...
		if(form.hasErrors())
			return badRequest(createQr.render(Login.getCustSession(), Login.loginForm, urlForm, InfoDisplay.ERROR, "The QrCode could not be generated. Please fill correctly all the requieted fields."));
		else {
			Url data = form.get();
			String url = data.url;
			...
			if (Pattern.matches(regex, url)) {
				try {
					...
					return ok(qrGenerator.render(Login.getCustSession(), domain + "r/" + qrId, Login.loginForm, InfoDisplay.SUCCESS, "You have successfully created a QrCode that redirects to " + url + " ."));
				}
				catch (Exception e) {
					return badRequest(createQr.render(Login.getCustSession(), Login.loginForm, urlForm, InfoDisplay.ERROR, "The QrCode could not be generated. " + e));
				}
			}
			else
				return badRequest(createQr.render(Login.getCustSession(), Login.loginForm, urlForm, InfoDisplay.ERROR, "The QrCode could not be generated. The redirection url must be a valid url."));
		}
	}
